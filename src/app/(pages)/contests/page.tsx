...
export default function ContestsPage() {
  ...
  const filteredAndSortedContests = useMemo(() => {
    const filtered = contests.filter((contest) => {
      ...
      const matchesPlatform = selectedPlatforms.length === 0 || selectedPlatforms.includes(contest.platform)
      const matchesDifficulty = !selectedDifficulty || contest.difficulty === selectedDifficulty
        ...
        <div className="flex flex-col space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">{filteredAndSortedContests.length} contests found</p>
            </div>
          ...
          <div className="flex-1">
            {filteredAndSortedContests.length === 0 ? (
              <div className="text-center py-12">
                <div className="text-muted-foreground text-lg">No contests found</div>
              ...
              >
                {filteredAndSortedContests.map((contest) => (
                  <ContestCard key={contest.id} contest={contest} onViewDetails={setSelectedContest} />
                ))}
              </div>
            )}
          </div>
